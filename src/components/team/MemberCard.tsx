...
export default function MemberCard({ member, priority = false }: MemberCardProps) {
  ...
  return (
    <div className="flex flex-col items-center w-[clamp(150px,20vw,250px)] min-h-[clamp(220px,28vw,350px)]">
      ...
      <p className="mt-2 text-center text-[clamp(1rem,2vw,1.5rem)] text-brand-orange font-body font-semibold">
        {member.name}
      </p>
      <p className="text-center text-[clamp(0.8rem,1.5vw,1rem)] text-brand-orange font-body font-medium">
        {member.position}
      </p>
      <p className="text-center text-[clamp(0.8rem,1.5vw,1rem)] text-brand-orange font-body">
        {member.degree}
      </p>
    </div>
  )
